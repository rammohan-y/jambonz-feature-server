  ...
  evaluateAllSessions() {
    if (this.activeTrackers) {
      for (const [sessionId, tracker] of this.activeTrackers.entries()) {
        const missingEvents = [];
        if (!tracker.events.playbackStart) missingEvents.push('playback-start');
        if (!tracker.events.playbackStop) missingEvents.push('playback-stop');
        if (!tracker.events.playResult) missingEvents.push('play result');
        if (!tracker.events.completedPlay) missingEvents.push('completed play file');
        ...
        if (missingEvents.length > 0) {
          ...
          this.logger.info({
            ...
            customerData
          }, `Playback tracking: Missing events for file ${tracker.filename}: ${missingEvents.join(', ')}`);
        } else {
...
class TaskSay extends TtsTask {
  ...
  generateSessionId() {
    return `${Date.now()}-${this.sessionCounter++}`;
  }
  ...
  async handling(cs, {ep}) {
    ...
    // initialize playback tracker with cs
    if (!this.playbackTracker) {
      this.playbackTracker = new PlaybackTracker(this.logger, cs);
    }
    ...
    while (!this.killed && (this.loop === 'forever' || this.loop--) && ep?.connected) {
      ...
      while (!this.killed && segment < filepath.length) {
        if (cs.isInConference) {
          ...
          this.playbackTracker.startTracking(sessionId, filename);

          ep.once('playback-start', (evt) => {
            this.logger.info({evt}, 'Say got playback-start');
            this.playbackTracker.markEvent(sessionId, 'playbackStart');
            if (this.otelSpan) {
          ...
          ep.once('playback-stop', (evt) => {
            this.logger.info({evt}, 'Say got playback-stop');
            this.playbackTracker.markEvent(sessionId, 'playbackStop');
          ...
          this.logger.info({r}, 'Say:exec play result');
          this.playbackTracker?.markEvent(sessionId, 'playResult');
          try {
            // wait for playback-stop event received to confirm if the playback is successful
          ...
          if (filepath[segment].startsWith('say:{')) {
            ...
            this.logger.info(`Say:exec completed play file ${filepath[segment]}`);
            this.playbackTracker?.markEvent(sessionId, 'completedPlay');
          }
        }
        segment++;
      }
