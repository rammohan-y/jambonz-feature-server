...
class TaskGather extends SttTask {
  constructor(logger, opts, parentTask) {
    ...
    this.sayHasText = false;
    // Track AWS interim results for timeout logging
    this._awsInterimResultReceived = false;
  ...
  async handling(cs, {ep}) {
    ...
    try {
      if (this.sayTask) {
        ...
        if (this.sayTask?.text) {
          this.sayHasText = true;
        }
  ...
  _onTranscription(cs, ep, evt, fsEvent) {
    ...
    if (evt.is_final) {
      /* track AWS final results for call-level statistics */
      if (this.sayHasText) {
        this.cs._awsFinalResultCount++;
      }
      ...
      /* interim transcript */
      /* track AWS interim results for timeout logging and call-level statistics */
      if (this.sayHasText && !evt.is_final) {
        this._awsInterimResultReceived = true;
      }
  ...
  _onVadDetected(cs, ep) {
    ...
    this.callSession._voiceDetected = true;
    if (this.bargein && this.minBargeinWordCount === 0) {
      this._killAudio(cs);
      this.emit('vad');
    }
  ...
  async _resolve(reason, evt) {
    ...
    if (reason.includes('dtmf-interdigit-timeout') || reason.includes('dtmf-terminator-key')
      || reason.includes('dtmf-num-digits')) {
      this.cs._dtmfFinalResultCount++;
    }
    if (this.needsStt && this.ep && this.ep.connected) {
      // Handle AWS timeout scenarios
      if (this.sayHasText && reason.includes('timeout')) {
        if (this._awsInterimResultReceived) {
          this.cs._awsInterimResultCount++;
          // Timeout occurred but we had received interim results
          this.logger.info('interim result received with is_final=false and stop transcribe was issued');
          // Note: _awsInterimResultCount was already incremented when interim results were received
        } else {
          // Timeout occurred with no interim results received
          this.logger.info('no interim result received with is_final=false and stop transcribe was issued');
          this.cs._awsTimeOutResult++;
        }
      }
