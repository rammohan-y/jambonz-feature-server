...
class CallSession extends Emitter {
  ...
  constructor({logger, application, srf, tasks, callInfo, accountInfo, rootSpan, memberId, confName, confUuid}) {
    ...
    this.playbackTracker = new PlaybackTracker(logger, this);
    ...
    this._dtmfFinalResultCount = 0;
    this._gatherCount = 0;
    this._awsInterimResultCount = 0;
    this._awsFinalResultCount = 0;
    this._awsTimeOutResult = 0;
    this.sipReferCount = 0;
    this.delayedResponseFromBotCount = 0;
    this.jambonzErrorCount = 0;
    this.isAgentAssist = false;
...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  async _clearResources() {
    this.ep?.removeCustomEventListener('aws_transcribe::no_audio_detected',
      this.awsTranscribeNoAudioDetected.bind(this));
    this.ep?.removeCustomEventListener('aws_transcribe::vad_detected', this.awsTranscribeVadDetected.bind(this));
    this.logger.info({customerData: this.callInfo?.customerData}, 'Call had voice detected ' +
    `${this._voiceDetected}, call duration: ${this.callInfo?.duration}`);
    const callData = {
      'callTerminationBy': this.callInfo.callTerminationBy === 'jambonz' ? 'SAVG' : this.callInfo.callTerminationBy,
      'voiceDetected': this._voiceDetected,
      'gatherCount': this._gatherCount,
      'dtmfFinalResultCount': this._dtmfFinalResultCount,
      'awsInterimResultCount': this._awsInterimResultCount,
      'awsFinalResultCount': this._awsFinalResultCount,
      'awsTimeOutResult': this._awsTimeOutResult,
      'callDuration': this.callInfo?.duration,
      'transferedCount': this.sipReferCount,
      'delayedResponseFromBotCount': this.delayedResponseFromBotCount,
      'savgErrorCount': this.jambonzErrorCount,
      'isAgentAssist': this.isAgentAssist
    };
    // Log AWS transcription statistics at call cleanup
    if (this._gatherCount > 0) {
      if (this._voiceDetected) { // voice detected, so need to worry about transcripts
        if (this._awsFinalResultCount === 0 && this._awsInterimResultCount === 0) {
          this.logger.info({customerData: this.callInfo?.customerData, callData},
            `Call had audio but no transcript: ${this._awsInterimResultCount} interim results with ` +
            `${this._awsTimeOutResult} timeouts, ${this._awsFinalResultCount} final results, ` +
            `${this._dtmfFinalResultCount} dtmf results, voice detected: ${this._voiceDetected} ` +
            `gatherCount: ${this._gatherCount}, call duration: ${this.callInfo?.duration}, ` +
            `transferedCount: ${this.sipReferCount}, ` +
            `delayedResponseFromBotCount: ${this.delayedResponseFromBotCount}, ` +
            `savgErrorCount: ${this.jambonzErrorCount}`);
        } else {
          this.logger.info({customerData: this.callInfo?.customerData, callData},
            `Call had audio and transcript: ${this._awsInterimResultCount} interim results with ` +
            `${this._awsTimeOutResult} timeouts, ${this._awsFinalResultCount} final results, ` +
            `${this._dtmfFinalResultCount} dtmf results, voice detected: ${this._voiceDetected} ` +
            `gatherCount: ${this._gatherCount}, call duration: ${this.callInfo?.duration}, ` +
            `transferedCount: ${this.sipReferCount}, ` +
      ...
      } else if (this._dtmfFinalResultCount > 0) { // dtmf results, so no need to worry about transcripts
        this.logger.info({customerData: this.callInfo?.customerData, callData},
          `Call had dtmf results: ${this._awsInterimResultCount} interim results with ` +
          `${this._awsTimeOutResult} timeouts, ${this._awsFinalResultCount} final results, ` +
          `${this._dtmfFinalResultCount} dtmf results, voice detected: ${this._voiceDetected} ` +
          `gatherCount: ${this._gatherCount}, call duration: ${this.callInfo?.duration}, ` +
          `transferedCount: ${this.sipReferCount}, ` +
          `delayedResponseFromBotCount: ${this.delayedResponseFromBotCount}, ` +
          `savgErrorCount: ${this.jambonzErrorCount}`);
      } else if (!this._voiceDetected) { // voice not detected, no dtmf results then log
        // check if gathercount is only 1 and call duration is less than the gather's timeout
        // if yes, the call terminated by user without finishing the first gather
        let shouldLog = true;
        if (this.callInfo?.duration <= (this.gatherTimeout / 1000)) {
          this.logger.info({customerData: this.callInfo?.customerData, callData},
            `Call terminated early: voice detected: ${this._voiceDetected}, gatherCount: ` +
            `${this._gatherCount}, call duration: ${this.callInfo?.duration}, ` +
            `transferedCount: ${this.sipReferCount}, ` +
            `delayedResponseFromBotCount: ${this.delayedResponseFromBotCount}, ` +
            `savgErrorCount: ${this.jambonzErrorCount}`);
          shouldLog = false;
        ...
        if (shouldLog) {
          this.logger.info({customerData: this.callInfo?.customerData, callData},
            `Call had no transcript: voice detected: ${this._voiceDetected}, gatherCount: ${this._gatherCount}, ` +
            `call duration: ${this.callInfo?.duration}, transferedCount: ${this.sipReferCount}, ` +
            `delayedResponseFromBotCount: ${this.delayedResponseFromBotCount}, ` +
            `savgErrorCount: ${this.jambonzErrorCount}`);
        }
      }
    } else { // no gather command, so no need to worry about transcripts
      this.logger.info({customerData: this.callInfo?.customerData, callData}, 'Call had no gather commands ' +
      `gatherCount: ${this._gatherCount}, call duration: ${this.callInfo?.duration} ` +
      `transferedCount: ${this.sipReferCount}, ` +
      `delayedResponseFromBotCount: ${this.delayedResponseFromBotCount}, ` +
      `savgErrorCount: ${this.jambonzErrorCount}`);
  ...
  _notifyTaskError(obj) {
    if (this.cs) this.cs.jambonzErrorCount++;
    if (this.requestor instanceof WsRequestor) {
  ...
  awsTranscribeNoAudioDetected(evt) {
    this.logger.info('Received only silence during aws transcription');
  }
  awsTranscribeVadDetected(evt) {
    this.logger.info('Received vad_detected during aws transcription');
    this._voiceDetected = true;
  }
  async _createMediaEndpoint(drachtioFsmrfOptions = {}) {
    const ep = await createMediaEndpoint(this.srf, this.logger, {
      activeMs: this.getMS(),
    ...
    ep.addCustomEventListener('aws_transcribe::no_audio_detected', this.awsTranscribeNoAudioDetected.bind(this));
    ep.addCustomEventListener('aws_transcribe::vad_detected', this.awsTranscribeVadDetected.bind(this));
    return ep;
  }
...
class PlaybackTracker {
  ...
  startTracking(sessionId, filename) {
    const tracker = {
      filename,
      sessionId,
      startTime: Date.now(),
    ...
    this.logger.debug(`Started tracking playback session ${sessionId} for file: ${filename}`);
  }

  markEvent(sessionId, eventType) {
    const tracker = this.activeTrackers.get(sessionId);
    if (tracker) {
      tracker.events[eventType] = true;
      this.logger.debug(`Marked event ${eventType} for session ${sessionId}`);

      // Check if all events are completed
      if (this.allEventsCompleted(tracker)) {
        this.logger.debug(`All events completed for session ${sessionId}, file: ${tracker.filename}`);
        this.activeTrackers.delete(sessionId);
      }
    }
  }

  allEventsCompleted(tracker) {
    return Object.values(tracker.events).every((completed) => completed);
  }

  evaluateAllSessions() {
    if (this.activeTrackers) {
      for (const [sessionId, tracker] of this.activeTrackers.entries()) {
        const missingEvents = [];
        // during race conditions, playbackStart event might not have come
        // but if either playbackResult or completedPlay event has come,
        // we can consider the task successful
        if (!tracker.events.playbackStart && !tracker.events.playbackResult &&
          !tracker.events.completedPlay) {
          missingEvents.push('playback-start');
        }
        // during race conditions, playbackStop event might not have come
        // but if either playbackResult or completedPlay event has come,
        // we can consider the task successful
        if (!tracker.events.playbackStop && !tracker.events.playbackResult
          && !tracker.events.completedPlay) {
          missingEvents.push('playback-stop');
        }
        // if playbackStop event has come, no need to check for playResult event
        // the audio file has been played
        if (!tracker.events.playResult && !tracker.events.playbackStop) {
          missingEvents.push('play result');
        }

        if (missingEvents.length > 0 && this.cs?.sipReferCount === 0) {
          const callData = {
            'callTerminationBy': this.cs?.callInfo?.callTerminationBy === 'jambonz' ? 'SAVG'
              : this.cs?.callInfo?.callTerminationBy,
            'voiceDetected': this.cs?._voiceDetected,
            'gatherCount': this.cs?._gatherCount,
            'dtmfFinalResultCount': this.cs?._dtmfFinalResultCount,
            'awsInterimResultCount': this.cs?._awsInterimResultCount,
            'awsFinalResultCount': this.cs?._awsFinalResultCount,
            'awsTimeOutResult': this.cs?._awsTimeOutResult,
            'callDuration': this.cs?.callInfo?.duration,
            'transferedCount': this.cs?.sipReferCount,
            'delayedResponseFromBotCount': this.cs?.delayedResponseFromBotCount,
            'savgErrorCount': this.cs?.jambonzErrorCount,
            'isAgentAssist': this.cs?.isAgentAssist
          };
          // 'cs' is not available in this context, so we cannot add callInfo?.customerData here.
          ...
          this.logger.info({
            ...
            customerData,
            callData
          }, `Playback tracking: Missing events for file ${tracker.filename} sessionId: ${tracker.sessionId} : ` +
            `${missingEvents.join(', ')}`);
        } else {
          this.logger.debug('Playback tracking: All events completed for session ' +
            `${sessionId}, file: ${tracker.filename} elapsedTime: ${Date.now() - tracker.startTime}`);
        }
      }
    }
  }
  ...
  destroy() {
    // Evaluate all remaining sessions before cleanup
    this.evaluateAllSessions();
