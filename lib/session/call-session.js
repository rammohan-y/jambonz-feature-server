 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  _clearResources() {
    ...
    if (this._awsFinalResultCount === 0) {
      ...
      this.logger.info(`Call had no transcript: ${this._awsInterimResultCount} interim results with  is_final=false, ${this._awsTimeOutResult} timeouts, ${this._awsFinalResultCount} final results`);
    } else {
      // eslint-disable-next-line max-len
      this.logger.info(`Call had transcript: ${this._awsInterimResultCount} interim results with  is_final=false, ${this._awsTimeOutResult} timeouts, ${this._awsFinalResultCount} final results`);
    }
