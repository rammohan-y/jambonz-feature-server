 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  _clearResources() {
    ...
    if (this._gatherCount > 0) {
      if (this._dtmfFinalResultCount > 0) { // dtmf results, so no need to worry about transcripts
        this.logger.info({customerData: this.callInfo?.customerData},
          `Call had dtmf results: ${this._awsInterimResultCount} interim results with  is_final=false, ` +
      ...
      } else if (this._voiceDetected) { // voice detected, so need to worry about transcripts
        if (this._awsFinalResultCount === 0 && this._awsInterimResultCount === 0) {
          this.logger.info({customerData: this.callInfo?.customerData},
            `Call had audio but no transcript: ${this._awsInterimResultCount} interim results with  is_final=false, ` +
            `${this._awsTimeOutResult} timeouts, ${this._awsFinalResultCount} final results, ` +
            `${this._dtmfFinalResultCount} dtmf results, voice detected: ${this._voiceDetected} ` +
            `gatherCount: ${this._gatherCount}, call duration: ${this.callInfo?.duration}`);
        } else {
          this.logger.info({customerData: this.callInfo?.customerData},
            `Call had audio and transcript: ${this._awsInterimResultCount} interim results with  is_final=false, ` +
            `${this._awsTimeOutResult} timeouts, ${this._awsFinalResultCount} final results, ` +
            `${this._dtmfFinalResultCount} dtmf results, voice detected: ${this._voiceDetected} ` +
            `gatherCount: ${this._gatherCount}, call duration: ${this.callInfo?.duration}`);
        }
      ...
      this.logger.info({customerData: this.callInfo?.customerData}, 'Call had no gather commands ' +
      `call duration: ${this.callInfo?.duration}`);
    }
