 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  _clearResources() {
    ...
    if (this._gatherCount > 0) {
      if (this._gatherCount == this._dtmfFinalResultCount) {
        this.logger.info({customerData: this.callInfo?.customerData},
          `Call had dtmf results: ${this._dtmfFinalResultCount}`
        );
      }
      else {
        if (this._awsFinalResultCount === 0 && this._awsTimeOutResult > 0 &&
          this._dtmfFinalResultCount === 0 && !this._voiceDetected) {
          this.logger.info({customerData: this.callInfo?.customerData},
            `Call had no transcript: ${this._awsInterimResultCount} interim results with  is_final=false, ` +
          ...
          if (this._awsFinalResultCount > 0) {
            this.logger.info({customerData: this.callInfo?.customerData},
              `Call had transcript: ${this._awsInterimResultCount} interim results with  is_final=false, ` +
              `${this._awsTimeOutResult} timeouts, ${this._awsFinalResultCount} final results, ` +
              `voice detected: ${this._voiceDetected}`);
          }
        }
      }
    }
