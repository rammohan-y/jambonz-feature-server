...
const createMediaEndpoint = async(srf, logger, {
  ...
  inbandDtmfEnabled,
  mediaTimeoutHandler,
  awsTranscribeNoAudioDetected,
  awsTranscribeVadDetected
} = {}) => {
  ...
  if (awsTranscribeNoAudioDetected) {
    ep.addCustomEventListener('aws_transcribe::no_audio_detected', awsTranscribeNoAudioDetected);
  }
  if (awsTranscribeVadDetected) {
    ep.addCustomEventListener('aws_transcribe::vad_detected', awsTranscribeVadDetected);
  }
